...
infected_to_recovered = function(day_number, i){ #ill for 2+ days
  result = 0
  if (day_number >= 4){
    j = day_number - 1
    days = -1
    given_day = all_list[[j]]
    while (j >= 0 && (given_day[[i]]$status == "I" || given_day[[i]]$status == "Q")){
      days = days + 1
      if (j != 0){
        given_day = all_list[[j]]
      }
...
t = total_days #ctrl + shift + c
print(susceptible_list_no_vaccine[t])
print(susceptible_list_vaccine[t])
print(infected_list[t]) #starts day 1
print(quarantined_list[t]) #starts day 2 (after ill 1st day)
print(recovered_list[t]) #starts day 4 (ill for 3+)
print(deaths_list[t]) #starts day 4 (ill for 3+)
